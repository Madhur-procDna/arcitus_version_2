...
class Settings(BaseSettings):
    ...
    data_file_path: str = "data/Arcutis Dummy Data v1.xlsx"
    ...
    cors_origins: list[str] = Field(
        default_factory=lambda: [
            o.strip()
            for o in (
                os.environ.get("CORS_ALLOW_ORIGINS", "http://localhost:3000,http://127.0.0.1:3000")
            ).split(",")
            if o.strip()
        ],
    )
    fastapi_base_url: str = Field(
        default_factory=lambda: os.environ.get("FASTAPI_BASE_URL", "http://localhost:8000"),
    )

    model_config = SettingsConfigDict(
        env_file=(".env", str(_BACKEND_ROOT / ".env")),
    ...
    def _normalize_data_file_path(self) -> "Settings":
        ...
        if self.sda_data_source.lower() == "sqlite" and not Path(self.data_file_path).exists():
            raise ValueError(
                "SQLite mode requires a readable DATA_FILE_PATH. Set DATA_FILE_PATH to a valid workbook path."
            )
        return self
