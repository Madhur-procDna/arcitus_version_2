...
function stripMetaForStorage(meta: RecentChatMessage['meta']): RecentChatMessage['meta'] {
  ...
  const { chart, result_table, ...lightMetaBase } = meta;
  const lightMeta = { ...lightMetaBase };
  delete lightMeta.sql;
    ...
    ...(chart
      ? {
          chart: {
            kind: chart.kind,
            data: [],
            ...(typeof chart.title === 'string' && chart.title ? { title: chart.title } : {}),
            ...(typeof chart.description === 'string' && chart.description
              ? { description: chart.description }
              : {}),
            ...(chart.showGrowthLines === true ? { showGrowthLines: true } : {}),
          },
        }
      : {}),
...
function serializeMessages(messages: RecentChatMessage[]): RecentChatMessage[] {
  return messages.map((m) => ({
    role: m.role,
    // Limit retained content to reduce sensitive data persistence in browser storage.
